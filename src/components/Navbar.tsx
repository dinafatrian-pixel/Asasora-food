...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header role="banner" className="bg-white/95 backdrop-blur-md sticky top-0 z-50 shadow-2xs border-b border-emerald-900/10 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <nav className="hidden lg:flex items-center space-x-1 xl:space-x-2">
            {navLinks.map((link) => (
              <button
                ...
                className="px-3 py-2 rounded-xl text-sm font-semibold text-gray-700 hover:text-[#2E6F40] hover:bg-emerald-50/80 transition-all duration-150 cursor-pointer"
              >
          ...
          <div className="hidden lg:flex items-center space-x-3">
            ...
            <div className="flex items-center bg-gray-100/90 border border-gray-200/80 rounded-full p-1 text-xs font-bold text-gray-600 shadow-2xs">
              <button
                type="button"
                onClick={() => setLang('id')}
            ...
            <button
              ...
              className="bg-[#2E6F40] hover:bg-emerald-800 text-white font-extrabold px-4 py-2.5 rounded-xl text-xs sm:text-sm flex items-center gap-2 shadow-sm transition-all duration-150 cursor-pointer active:scale-95 border border-emerald-600"
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="bg-[#F3C623] text-gray-900 text-[11px] font-black px-2 py-0.5 rounded-full leading-none animate-pulse">
                  {cartCount}
                </span>
              )}
            </button>
            ...
            <a
              ...
              className="bg-[#F3C623] hover:bg-[#D1A310] text-gray-900 font-extrabold pl-2 pr-3.5 py-1.5 rounded-full shadow-2xs transition flex items-center space-x-1.5 text-xs cursor-pointer active:scale-95 border border-[#e5b719]"
            >
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-b border-gray-100 px-5 pt-3 pb-5 space-y-1 shadow-lg animate-in slide-in-from-top-2">
          {navLinks.map((link) => (
            <button
              key={link.href}
              onClick={() => handleNavClick(link.href)}
