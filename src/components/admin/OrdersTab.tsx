...
import { formatRupiah, generateOrderId } from '../../utils/distance';
    ...
    <div className="space-y-4" id="admin-orders-tab">
      ...
      {viewingInvoiceOrder && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="relative w-full max-w-3xl my-8 bg-transparent">
            <button
              onClick={() => setViewingInvoiceOrder(null)}
              className="absolute top-4 right-4 z-10 bg-white/90 hover:bg-white text-gray-800 p-2 rounded-full shadow-lg transition cursor-pointer"
              title="Tutup Invoice"
            >
              <X className="w-5 h-5" />
            </button>
